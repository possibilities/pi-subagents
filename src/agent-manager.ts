      ...
      .then(({ responseText, session, aborted, steered, failure }) => {
        ...
        if (record.status !== "stopped") {
          if (aborted) {
            ...
            // No usable output — surface the terminal stop reason (provider
            // error / abort / empty length stop) as a failure instead of a
            // dishonest "completed" with an empty result.
            record.status = "error";
  ...
  async resume(
    ...
    try {
      ...
      if (responseText) {
        ...
        // Same honesty rule as the spawn path: an empty resume result is a
        // terminal failure carrying the stop reason, not a silent completion.
        record.status = "error";
